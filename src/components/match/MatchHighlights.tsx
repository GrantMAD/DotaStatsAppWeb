...
import { LOBBY_TYPES, REGIONS } from '@/services/constants';
...
interface HighlightItemProps {
  icon: LucideIcon;
  title: string;
  player: MatchDetails['players'][number];
  value: string;
  colorClass: string;
}
...
function HighlightItem({
  ...
  return (
    <GlassCard className="p-6 flex items-center gap-6 group hover:bg-white/5 transition-all">
      <div
        className={cn(
          'p-4 rounded-2xl bg-white/5 shadow-lg group-hover:scale-110 transition-transform duration-500',
          colorClass
        )}
      >
        <Icon size={32} />
...
export function MatchHighlights({ match }: { match: MatchDetails }) {
  const getHighlights = (m: MatchDetails) => {
    ...
    const topDamage = [...sortedPlayers].sort(
      (a, b) => b.hero_damage - a.hero_damage
    )[0];

    const topNetWorth = [...sortedPlayers].sort(
      (a, b) => b.net_worth - a.net_worth
    )[0];

    const topTowers = [...sortedPlayers].sort(
      (a, b) => b.tower_damage - a.tower_damage
    )[0];

    const topHealing = [...sortedPlayers].sort(
      (a, b) => b.hero_healing - a.hero_healing
    )[0];

    const topStacks = [...sortedPlayers].sort(
      (a, b) => (b.camps_stacked || 0) - (a.camps_stacked || 0)
    )[0];

    const topWards = [...sortedPlayers].sort(
      (a, b) =>
        ((b.obs_placed || 0) + (b.sen_placed || 0)) -
        ((a.obs_placed || 0) + (a.sen_placed || 0))
    )[0];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {((h.topWards.obs_placed || 0) +
          (h.topWards.sen_placed || 0)) > 0 && (
          <HighlightItem
            icon={Eye}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <GlassCard className="lg:col-span-2 p-8">
          <h3 className="text-sm font-black text-white uppercase tracking-widest mb-8 flex items-center gap-3">
            <Info size={18} className="text-gaming-accent" />
            Match Metadata
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <span className="text-sm font-black text-white">
                  {LOBBY_TYPES[match.lobby_type] || 'Standard'}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <span className="text-sm font-black text-white">
                  {REGIONS[match.region] || `Region ${match.region}`}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                ...
                <span className="text-sm font-black text-white">
                  {match.patch ? `Patch ${match.patch}` : '7.35'}
                </span>
              </div>
        ...
        {!match.version && (
          <GlassCard className="p-8 border-dashed border-white/10 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-500 font-medium text-xs leading-relaxed">
              Additional performance metrics like &quot;Master Stacker&quot;
              and &quot;Support Impact&quot; are only available for fully
              parsed matches.
            </p>
          </GlassCard>
        )}
