...
import { LayoutGrid, BarChart2, Timer, MessageSquare, Trophy, AlertCircle, Lock } from 'lucide-react';
...
export function MatchDetailModal({ isOpen, onClose, matchId }: MatchDetailModalProps) {
    ...
    >
      {isLoading ? (
        <div className="space-y-6 animate-pulse p-4">
          <div className="h-32 w-full bg-[var(--nav-hover)] rounded-3xl" />
          <div className="h-12 w-full bg-[var(--nav-hover)] rounded-xl" />
          <div className="space-y-3">
            {[1, 2, 3, 4].map(i => <div key={i} className="h-16 w-full bg-[var(--nav-hover)] rounded-xl" />)}
          </div>
        </div>
      ) : error || !match ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <AlertCircle className="w-16 h-16 text-loss mb-4 opacity-20" />
          <h3 className="text-xl font-black text-foreground uppercase tracking-tight">Match Data Unavailable</h3>
          <p className="text-gray-500 max-w-xs mx-auto mt-2">
            This match might be too old or private. Full details are not available.
          </p>
          <Button 
            variant="secondary" 
            className="mt-6"
        ...
        <div className="space-y-6">
          {/* Header Summary */}
          <div className="bg-[var(--nav-hover)] rounded-3xl p-6 border border-[var(--card-border)]">
             <div className="flex items-center justify-between gap-8">
                <div className="flex-1 text-center">
                   <p className={cn("text-3xl font-black italic", match.radiant_win ? "text-win" : "text-gray-500")}>
                     {match.radiant_score}
                   </p>
                   <p className="text-[10px] font-black uppercase text-gray-600 tracking-widest mt-1">Radiant</p>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                   ...
                   <div className={cn(
                     ...
                     match.radiant_win ? "bg-win/20 text-win border-win/30" : "bg-loss/20 text-loss border-loss/30"
                   )}>
                ...
                <div className="flex-1 text-center">
                   ...
                   <p className="text-[10px] font-black uppercase text-gray-600 tracking-widest mt-1">Dire</p>
                </div>
             </div>
          </div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-1 min-w-[100px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl transition-all duration-300 font-black uppercase text-[10px] tracking-widest",
                    isActive 
