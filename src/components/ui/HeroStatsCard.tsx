...
export function HeroStatsCard({
  ...
  return (
    <AnimationWrapper animationType="fade-in">
      <GlassCard hoverable className="w-45 shrink-0 p-3 flex flex-col gap-3 group relative overflow-hidden">
        {tier && (
          <div className={cn(
            "absolute top-0 right-0 w-12 h-12 flex items-center justify-center font-black text-xl italic rounded-bl-2xl border-l border-b border-(--card-border) z-10",
        ...
        {rank && (
          <div className="absolute top-2 left-2 bg-(--card-bg) backdrop-blur-md w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white z-10 border border-(--card-border)">
            {rank}
          </div>
        )}

        <div className="relative aspect-video rounded-lg overflow-hidden bg-(--nav-hover) border border-(--card-border)">
          <Image
            src={imgUrl}
            alt={heroName}
            width={320}
        ...
        <div className="mt-auto">
          <div className="w-full bg-(--nav-hover) h-1.5 rounded-full overflow-hidden">
            <div
              className={cn(
