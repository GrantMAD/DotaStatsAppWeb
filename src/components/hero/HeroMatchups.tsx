...
function MatchupItem({ heroId, winRate, isStrong }: MatchupItemProps) {
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-xl bg-(--nav-hover) border border-(--card-border) group hover:border-gaming-accent/30 transition-all">
      <div className="flex items-center gap-3">
        <div className="relative w-10 h-10">
          ...
          <div
            className={cn(
              "absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-(--card-bg) shadow-sm",
        ...
        <div>
          ...
          <div className="flex gap-1 mt-0.5">
            {hero.roles.slice(0, 2).map(role => (
              <span
                key={role}
                className="text-[8px] font-bold text-gray-500 uppercase tracking-tighter"
              >
                {role}
              </span>
            ))}
...
export function HeroMatchups({ heroId }: { heroId: number }) {
  ...
  const { strongAgainst, weakAgainst } = useMemo(() => {
    const sorted = [...matchups].sort(
      (a, b) =>
        b.wins / b.games_played - a.wins / a.games_played
    );

    return {
      strongAgainst: sorted
        .slice(0, 5)
        .map(m => ({
          id: m.hero_id,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[1, 2].map(i => (
          <div key={i} className="space-y-4">
            ...
            {[1, 2, 3, 4, 5].map(j => (
              <Skeleton key={j} className="w-full h-14 rounded-xl" />
            ))}
          </div>
        ))}
      </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-black uppercase tracking-widest text-win italic">
            Strong Against
          </h3>
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-black uppercase tracking-widest text-loss italic">
            Weak Against
          </h3>
        </div>
